...
- `<thead>`: Dùng để thể hiện tiêu đề của bảng, thường chỉ đuợc dùng 1 lần để ghi nội dung tiêu đề. -`<tbody>`: Dùng để chứa nội dung chính của bảng, có thể được dùng nhiều lần trong 1 bảng để chia dữ liệu thành các nhóm. -`<tfoot>`: Dùng để thể hiện các dong tổng kết các dữ liệu trong bảng(tổng tiền, tổng sinh viên, etc), thường xuất hiện ở dưới cùng của bảng.
...
* Không tìm thấy thẻ `<table>` trên trang chủ do Shopee ưu tiên sử dụng hệ thống **Flexbox** và các thẻ `<div>` lồng nhau. Việc này giúp trang web linh hoạt hơn khi hiển thị trên nhiều kích thước màn hình khác nhau (Responsive) và dễ dàng tùy chỉnh CSS hơn so với cấu trúc bảng truyền thống.
