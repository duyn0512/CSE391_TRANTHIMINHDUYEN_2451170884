...
2. Phần tử cuối cùng hiển thị màu **DeepSkyBlue**
   - Vì:
     ...
     - Rule cuối cùng `#demo.text.highlight` có: - 1 ID (`#demo`) - 2 Class (`.text`, `.highlight`) - 0 Element
...
1. Sản phẩm A (h2)
   - `font-size` = 20px
     - Áp dụng trực tiếp từ bộ chọn `.card .title` có độ ưu tiên cao hơn các giá trị kế thừa.
   - `color` = green
     - Lớp `.highlight` chứ `!important`, ghi đè lên cả bộ chọn ID (`#featured .title`)

2. Mô tả sản phẩm ( p trong card featured)
   - `color` = blue
     - Thẻ `p` có thuộc tính `color: inherit`(kế thừa từ phần tử cha `.card`). Bộ chọn `.card { color: blue }` quy định màu của khối này là blue.

3. Sản phẩm B (h2)
   - `font-size` = 20px
     - Áp dụng trực tiếp từ quy tắc `.card .title`
   - `color` = blue
     - Thẻ `h2` kế thừa thuộc tính `color` từ phần tử cha `.card`.

4. Mô tả sản phẩm B (p.highlight)
   - `color` = green
     - Quy tắc `.highlight` chứa `!important` ghi đè lên mọi thuộc tính màu sắc khác.
